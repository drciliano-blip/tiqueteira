...
const LoteSchema = z
  .object({
    ...
    limitePorCpf: z.coerce.number().int().min(1).max(50).nullable(),
...
export async function salvarLote(
  ...
  const dados = LoteSchema.safeParse({
    ...
    // Campo vazio significa sem limite, que é o padrão.
    limitePorCpf: formData.get('limitePorCpf') || null,
